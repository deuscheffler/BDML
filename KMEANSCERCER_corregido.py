...
SQL_STORED_PROCEDURE = "sp_DatasetML"
...
# ============================================================================
def cargar_y_preparar(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Recibe el dataset ya cargado desde SQL Server y construye df_modelo (features codificadas)."""
    df = df.copy()

    # El SP sp_DatasetML devuelve 'nombre_categoria' en vez de 'categoria';
    # se renombra aquí para no tocar el resto del pipeline del modelo.
...
# ============================================================================
def detectar_outliers(df_modelo: pd.DataFrame) -> tuple[dict, dict]:
    """Detección de outliers univariados usando IQR.

    Retorna (outliers, limites): 'outliers' son las máscaras booleanas por
    columna (igual que antes); 'limites' guarda Q1/Q3/cota_inferior/
    cota_superior por columna, necesarios para recalcular es_outlier sobre
    un pedido nuevo en inferencia (Streamlit), donde no existe una serie
    completa sobre la cual calcular cuartiles.
    """
    def detectar_outliers_iqr(serie):
        Q1, Q3 = serie.quantile(0.25), serie.quantile(0.75)
        IQR = Q3 - Q1
        return (serie < Q1 - 1.5 * IQR) | (serie > Q3 + 1.5 * IQR), Q1, Q3, IQR
    ...
    for col in columnas_outliers:
        if col in df_modelo.columns:
            mask, Q1, Q3, IQR = detectar_outliers_iqr(df_modelo[col])
            outliers[col] = mask
            limites[col] = {
                "Q1": float(Q1),
                "Q3": float(Q3),
                "IQR": float(IQR),
                "cota_inferior": float(Q1 - 1.5 * IQR),
                "cota_superior": float(Q3 + 1.5 * IQR),
            }
...
def guardar_artefactos(
    ...
    # Obtener mapas de frecuencia para categorías (a partir del dataset ya cargado desde SQL Server)
    ...
    artefactos = {
        ...
        # Límites IQR por columna, para recalcular es_outlier en inferencia
        # sobre pedidos nuevos (Streamlit) que no forman parte de una serie.
...
# ============================================================================
# 10b. WRITE-BACK A SQL SERVER (cluster_kmeans, es_anomalia, es_outlier)
# ============================================================================
def preparar_resultados_no_supervisado(
    df: pd.DataFrame,
    labels: np.ndarray,
    anomalias: np.ndarray,
    df_modelo: pd.DataFrame,
) -> pd.DataFrame:
    """
    Arma el DataFrame con una fila por pedido (id_pedido, cluster_kmeans,
    es_anomalia, es_outlier) listo para escribirse en
    ResultadoModeloNoSupervisado. df, labels, anomalias y df_modelo
    comparten el mismo orden de filas (mismo índice original).
    """
    if "id_pedido" not in df.columns:
        raise ValueError(
            "'id_pedido' no está en el dataset cargado desde SQL Server; "
            "revisa que vw_ML_DataCoSupplyChain lo siga exponiendo."
        )

    resultados = pd.DataFrame({
        "id_pedido": df["id_pedido"].astype(int).values,
        "cluster_kmeans": np.asarray(labels).astype(int),
        "es_anomalia": np.asarray(anomalias).astype(bool),
        "es_outlier": df_modelo["es_outlier"].astype(bool).values,
    })

    print(f"Resultados preparados para write-back: {len(resultados)} filas")
    return resultados


def escribir_resultados_sql(
    engine: object,
    resultados: pd.DataFrame,
    tabla: str = "ResultadoModeloNoSupervisado",
) -> None:
    """
    Escribe los resultados del modelo no supervisado en SQL Server.
    Estrategia: TRUNCATE + INSERT, porque cada corrida de este script
    reentrena sobre el dataset completo y reemplaza las etiquetas
    anteriores (no es una carga incremental).
    """
    print(f"\nEscribiendo resultados en SQL Server ({tabla})...")
    try:
        with engine.begin() as conn:
            conn.execute(text(f"TRUNCATE TABLE {tabla}"))
            resultados.to_sql(
                tabla, conn, if_exists="append", index=False, chunksize=1000
            )
        print(f"✅ {len(resultados)} filas escritas en {tabla}")
    except Exception as e:
        print(f"❌ Error al escribir en {tabla}: {e}")
        print(
            "   Verifica que la tabla exista (04_ResultadoNoSupervisado.sql) "
            "y que el rol de conexión tenga permisos de escritura."
        )
        raise
...
def main() -> tuple[KMeans, pd.DataFrame, dict]:
    ...
    # 0. Conexión a SQL Server y carga de datos
    print("\n[0] Conectando a SQL Server y cargando datos...")
    engine = conectar_sql_server(SQL_SERVER, SQL_DATABASE)
    df_original = cargar_datos_desde_sql(engine, SQL_STORED_PROCEDURE)

    # 1. Carga y preparación
    df, df_modelo = cargar_y_preparar(df_original)
    ...
    _, limites_outliers = detectar_outliers(df_modelo)

    # 8b. Write-back a SQL Server: persiste cluster_kmeans / es_anomalia /
    # es_outlier en ResultadoModeloNoSupervisado para que
    # vw_ML_DataCoSupplyChain (y por lo tanto ModeloSupervisado.py) los use.
    print("\n[8b] Escribiendo resultados en SQL Server...")
    resultados_no_sup = preparar_resultados_no_supervisado(df, labels, anomalias, df_modelo)
    escribir_resultados_sql(engine, resultados_no_sup)
